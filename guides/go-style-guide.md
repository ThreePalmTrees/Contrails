...
Prefer descriptive names over abbreviations for variables, fields, parameters,
and receivers. A reader should be able to understand what a name refers to
without needing to look up its declaration.

**Exceptions** — the following short names are universally understood in Go and
...
func (a *App) startup(ctx context.Context) {
  ...
  go func() {
    ...
    for _, p := range projects {
      dur, err := a.processFile(p.path, p.dir)
    }
  }()
}
```
...
func (app *App) startup(ctx context.Context) {
  ...
  go func() {
    ...
    for _, project := range projects {
      duration, err := app.processFile(project.path, project.dir)
    }
  }()
}
```
...
For struct fields, the same principle applies:
